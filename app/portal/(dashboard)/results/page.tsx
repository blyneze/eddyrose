...
export default async function ResultsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {role === "SUPERADMIN" && <AdminResultView />}
      {role === "TEACHER" && <TeacherResultView userId={session.user.id} />}
      {role === "PARENT" && <ParentResultView userId={session.user.id} />}
    </div>
  )
}
...
// SUPER ADMIN VIEW
// ---------------------------------------------------------
async function AdminResultView() {
  const resultSheets = await getAdminResults()
...
// TEACHER VIEW
// ---------------------------------------------------------
async function TeacherResultView({ userId }: { userId: string }) {
  const { assignedClass, resultSheets } = await getTeacherResults(userId)
...
// PARENT VIEW
// ---------------------------------------------------------
async function ParentResultView({ userId }: { userId: string }) {
  const publishedResults = await getParentResults(userId)
